...
global main_db_name
global tmp_db_name
main_db_name = "student_database.db"
tmp_db_name = "tmp_database.db"
...
def add_student_to_student_database(id, name, class_name):
    connection = sqlite3.connect(main_db_name)
    cursor = connection.cursor()
    cursor.execute("INSERT INTO students VALUES ((?), (?), (?), 0)", (id, name, class_name,))
    connection.commit()
    connection.close()

def create_database():
    if not exists(main_db_name):
        connection = sqlite3.connect(main_db_name)
        cursor = connection.cursor()
        cursor.execute("""
            ...
            for cur_class_name in all_classes:
                ...
                with open(get_class_file_name(cur_class_name), "r") as file:
                    for student_name in file:
                        ...
                        id += 1
                        add_student_to_student_database(id, student_name, cur_class_name)
...
class Window(QWidget):
    ...
    def open_table_students(self, class_name):
        if self.table is not None:
            self.table.win.close()
            self.table = None
